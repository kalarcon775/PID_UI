...
def _get_tc08_dll_path() -> str:
    """
    Return the path to usbtc08.dll.

    - When running as a PyInstaller EXE, binaries are unpacked into sys._MEIPASS.
    - When running as normal .py/.pyw, we expect usbtc08.dll next to this file.
    """
    base_dir = getattr(sys, "_MEIPASS", os.path.dirname(__file__))
    return os.path.join(base_dir, "usbtc08.dll")

# Global default DLL path used by TC08Interface.__init__
DLL_PATH = _get_tc08_dll_path()

# Load the DLL once; reuse the handle in the class if you want
_tc08 = ctypes.WinDLL(DLL_PATH)
